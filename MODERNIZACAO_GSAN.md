...
2. **Mapa funcional por módulo** (➡ próxima atividade: **arrecadação**) — um documento por módulo em `docs/modernizacao/modulos/`. Ordem: ~~cadastro~~ ✅ → ~~micromedição~~ ✅ → ~~faturamento~~ ✅ → ~~cobrança~~ ✅ ([modulos/cobranca.md](docs/modernizacao/modulos/cobranca.md), 2026-08-14) → arrecadação → atendimento → segurança → batch → relatórios → integrações. O mapa da arrecadação deve fechar: movimento do arrecadador (retorno bancário/CNAB) → classificação → baixa nas identidades `*Geral`; aviso bancário/conciliação; pagamento parcial, excedente e não classificado; devolução/estorno; débito automático; PIX (`arrecadacao_pix`/`conta_qrcode_pix`); encerramento mensal da arrecadação.
