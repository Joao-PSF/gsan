...
2. **Mapa funcional por módulo** (➡ próxima atividade: **cobrança**) — um documento por módulo em `docs/modernizacao/modulos/`. Ordem: ~~cadastro~~ ✅ → ~~micromedição~~ ✅ → ~~faturamento~~ ✅ ([modulos/faturamento.md](docs/modernizacao/modulos/faturamento.md), 2026-08-14) → cobrança → arrecadação → atendimento → segurança → batch → relatórios → integrações. O mapa da cobrança deve fechar: estoque de débito (contas vencidas via ContaGeral), ações escalonadas por grupo/critério, corte/religação via OS, parcelamento completo (consolidação, prestações, descontos, desfazer), negativação SPC/Serasa e contas retidas/em revisão.
