...
2. **Mapa funcional por módulo** (➡ próxima atividade: **segurança**) — um documento por módulo em `docs/modernizacao/modulos/`. Ordem: ~~cadastro~~ ✅ → ~~micromedição~~ ✅ → ~~faturamento~~ ✅ → ~~cobrança~~ ✅ → ~~arrecadação~~ ✅ → ~~atendimento~~ ✅ ([modulos/atendimento.md](docs/modernizacao/modulos/atendimento.md), 2026-08-14) → segurança → batch → relatórios → integrações. O mapa da segurança deve fechar: RBAC completo (grupos, funcionalidades, operações, permissões especiais), **interação entre abrangência por localidade/gerência, unidade organizacional e permissão por funcionalidade** (fronteira levantada em atendimento.md §31), autenticação e histórico de senha, auditoria de operações e o modelo a reproduzir no SISAN.
