...
2. **Mapa funcional por módulo** (➡ próxima atividade: **relatórios**) — um documento por módulo em `docs/modernizacao/modulos/`. Ordem: ~~cadastro~~ ✅ → ~~micromedição~~ ✅ → ~~faturamento~~ ✅ → ~~cobrança~~ ✅ → ~~arrecadação~~ ✅ → ~~atendimento~~ ✅ → ~~segurança~~ ✅ → ~~batch~~ ✅ ([modulos/batch.md](docs/modernizacao/modulos/batch.md), 2026-08-14) → relatórios → integrações. O mapa dos relatórios deve fechar: catálogo e classificação (transacionais/operacionais/gerenciais/financeiros/regulatórios), geração síncrona × assíncrona pelo framework batch (`Relatorio`/`RelatorioGerado`, status e autorização), motor JasperReports e o serviço externo de relatórios, além da exceção de autorização das URLs contendo `relatorio`.
