...
2. **Mapa funcional por módulo** (➡ próxima atividade: **faturamento**) — um documento por módulo em `docs/modernizacao/modulos/`. Ordem: ~~cadastro~~ ✅ → ~~micromedição~~ ✅ ([modulos/micromedicao.md](docs/modernizacao/modulos/micromedicao.md), 2026-08-14) → faturamento → cobrança → arrecadação → atendimento → segurança → batch → relatórios → integrações. O mapa do faturamento deve fechar: orquestração do FATURAR_GRUPO (quem grava consumo), precedência das fontes de consumo mínimo, regra do valor de esgoto (percentual fotografado), mecanismo `*_geral`/`*_historico` e cálculo tarifário por categoria/economias/faixas.
