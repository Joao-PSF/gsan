...
Ordem recomendada (a revalidar após conclusão da Fase 0). Critério: começar por risco moderado e dependências simples; faturamento e arrecadação por último entre os críticos (risco financeiro); batch crítico ao final.
...
2. Módulo só é considerado migrado após: equivalência comprovada (testes de caracterização), validação de usuários, logs estáveis em produção e rollback disponível (Fase 12 por funcionalidade).
