...
🔵 Três conclusões funcionais fortes: (a) **pagar uma conta arquivada funciona** — a busca inclui o histórico, confirmando que a identidade do documento permanece válida na passagem corrente→histórico (fronteira do Faturamento resolvida); (b) **retificar não invalida o pagamento já vinculado** — o pagamento continua apontando a conta que efetivamente pagou, e RETIFICADA é situação classificável; note que a conta retificadora é **outro documento, com outra identidade** (ligada por origem), de modo que o pagamento não "migra" automaticamente para ela; (c) o sistema **não descarta** o pagamento quando o documento está prescrito/cancelado/parcelado: registra a razão numa situação específica, preservando o dinheiro para tratamento posterior.
