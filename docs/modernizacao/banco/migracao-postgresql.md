# PostgreSQL — OpenGSAN e Importação de Bases GSAN

> **Revisão 2026-09-15 (16ª execução)**: o sistema passa a chamar-se **OpenGSAN** e a **migração de instalações GSAN saiu do escopo deste projeto** (ADR-0005). A parte (3) deste documento deixa de ser "estratégia futura deste projeto" e passa a ser **insumo registrado para o projeto de migração**, separado.
>
> Revisão 2026-08-13 (2ª execução): a versão anterior planejava o corte de um banco de produção — premissa corrigida: **não há produção neste projeto**. O conteúdo técnico válido foi reorganizado em três partes: (1) o PostgreSQL do OpenGSAN; (2) restauração de bases GSAN para análise/referência; (3) conhecimento sobre migração — hoje fora de escopo.

## 1. PostgreSQL do OpenGSAN (desenvolvimento e VPS)
...
## 3. Migração GSAN → OpenGSAN (requisito arquitetural — **não implementar agora**)

Pipeline futuro, por companhia:

```text
Instalação GSAN → identificação da versão/schema → análise de compatibilidade
→ mapeamento GSAN→OpenGSAN → transformações necessárias → validação → migração → OpenGSAN
```
...
2. **Mapeamento registrado**: toda divergência estrutural do OpenGSAN em relação ao GSAN é registrada com sua transformação correspondente no documento de compatibilidade — o migrador nasce desse registro, não de engenharia reversa futura.
