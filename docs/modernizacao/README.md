...
| [compatibilidade/gsan-opengsan.md](compatibilidade/gsan-opengsan.md) | **Compatibilidade conceitual GSAN → OpenGSAN**: 145 conceitos centrais classificados em cinco classes (equivalente · equivalente com reorganização · divergência deliberada · sem equivalente · pendente), com a semântica declarada dos dois lados, a divergência `D-xx` referenciada e **o oráculo de teste que governa cada diferença**. Identidades, seis mecanismos de histórico, parametrização, variação por companhia e candidatos a divergência. ⚠️ **Conceitual — nunca operacional** |
...
- `compatibilidade/` — **completa**: o registro de divergências, a **análise de compatibilidade das estruturas centrais** (2026-09-14, *o que fazer com a estrutura existente*) e a **compatibilidade conceitual GSAN → OpenGSAN** (2026-09-15, *o que cada conceito passa a significar*). ⚠️ A estratégia operacional de migração **não** vive aqui — é projeto separado (ADR-0005).
- `modulos/` — **completa**: os dez mapas funcionais (cadastro → integrações), o **catálogo de funcionalidades futuras** e as **dependências e ordem de implementação** (2026-09-15). O resumo da ordem vive em `modulos/README.md`; a análise, em `modulos/dependencias-e-ordem-implementacao.md`.
