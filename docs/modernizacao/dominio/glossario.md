...
- **Evidências**:
  ...
  ```
- **Observações de modernização**: mecanismo compreendido (2026-08-14): `ContaGeral` é a entidade física fonte do id estável (`seq_conta_geral` + `indicadorHistorico`), 1:1 com a conta corrente, a histórica e a impressão; retificação cria **nova** conta encadeada por `origem`; cancelamento é transição de estado com motivo; arquivamento nos batches de encerramento mensal. Semântica a preservar no SISAN — ver [modulos/faturamento.md §13–15](../modulos/faturamento.md).
...
4. ~~Mecanismo `*_geral`/`*_historico`~~ **Resolvida (2026-08-14)**: a entidade `*_geral` é física e é a **fonte da identidade** (sequence própria + `indicadorHistorico`), com 1:1 para o documento corrente e o histórico (mesmo id); referências externas (pagamento, cobrança) apontam a identidade e sobrevivem ao arquivamento — que ocorre nos batches de **encerramento mensal**. Retificação cria novo documento encadeado por `origem`. Ver [modulos/faturamento.md §13–15](../modulos/faturamento.md). A caracterização numérica continua prevista na Fase 2.
